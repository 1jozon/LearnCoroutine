...
static thread_local Fiber* t_fiber = nullptr; // 当前线程上的协程
...
static std::atomic<uint64_t> s_fiber_id{0}; // 协程计数器
static std::atomic<uint64_t> s_fiber_count{0}; // 协程id
...
void Fiber::MainFunc()
{
    ...
    assert(curr != nullptr);
    if (debug) std::cout << "Fiber::MainFunc() id = " << curr->getId() << std::endl;
    curr->m_cb(); // 执行协程函数
    curr->m_cb = nullptr; // 清空协程函数
    curr->m_state = TERM; // 设置状态为挂起
